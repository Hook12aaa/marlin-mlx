...
What I did not expect was that the hardest part would not be the LLM or the optimisation. It would be the vision pipeline: getting the right frames, in the right order, with the right position encoding, so the ViT produces embeddings the LLM can use.
